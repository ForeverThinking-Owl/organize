...
当前版本在 v0.3.5 Skill Runtime 语义硬化基础上，把 `MemoryStore` 接入 `ActorRuntime` 的运行生命周期：调用方可以通过 `runtimeOptions.memoryStore` 发起 store-backed run；Runtime 会在构建 `ActorContext` 前自动 load / restore 记忆快照，并在记忆生成后自动 dump / save 快照，同时用 Trace 暴露 `memory_store_load`、`memory_store_save`、`memory_store_error` 生命周期事件。
...
从 v0.3.6 开始，调用方可以把 Store 交给 Runtime：
...
这意味着 Runtime 会在构建上下文前加载历史记忆，让旧经验参与本次检索；运行结束沉淀新记忆后，再保存新的快照。后续仍然可以在不改动记忆核心语义的前提下，替换具体存储实现。

## 项目状态

当前仍是实验性 Actor Kernel。它的重点不是把所有能力一次做完，而是逐步明确边界：Actor、Skill、Tool、Policy、Approval、Trace、Memory、Store 都先形成最小可运行闭环，再逐步硬化。

下一阶段将继续沿着运行时正确性推进，优先处理 Trace 持久化、显式 human input / wait approval 运行语义，以及更通用的 Skill 执行语义。
...
This version binds `MemoryStore` into the `ActorRuntime` run lifecycle on top of v0.3.5 Skill Runtime Semantics. Callers can start a store-backed run through `runtimeOptions.memoryStore`; the Runtime loads and restores the memory snapshot before building `ActorContext`, then dumps and saves the snapshot after memory generation. Trace now exposes `memory_store_load`, `memory_store_save`, and `memory_store_error` lifecycle events.
...
Starting in v0.3.6, callers can hand the Store to Runtime:
...
The Runtime loads historical memory before context construction so past experience can participate in retrieval, then saves a fresh snapshot after new memory is crystallized. This still preserves the storage boundary, so future implementations can replace local JSON without changing core memory semantics.
...
The next stage will continue improving runtime correctness, especially Trace persistence, explicit human input / wait approval runtime semantics, and more general Skill execution semantics.
