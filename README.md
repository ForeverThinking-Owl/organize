...
当前版本在 v0.3.6 Runtime Store Binding 基础上，为 Trace 增加快照与存储边界：`TraceLogger` 支持 `dumpSnapshot()` / `restoreSnapshot()`；新增 `TraceSnapshot`、`TraceStore` 与 `JsonTraceStore`；Trace 可以保存为本地 JSON 快照，并在 clear / restore 后继续复盘 ActorRunTrace 的状态、事件顺序和关键审计事件。
...
下一阶段将继续沿着运行时正确性推进，优先处理显式 human input / wait approval 运行语义，以及更通用的 Skill 执行语义。
...
This version adds snapshot and storage boundaries for Trace on top of v0.3.6 Runtime Store Binding. `TraceLogger` now supports `dumpSnapshot()` / `restoreSnapshot()`; `TraceSnapshot`, `TraceStore`, and `JsonTraceStore` are available; Trace can be saved to a local JSON snapshot and restored after clear, preserving ActorRunTrace status, event ordering, and key audit events.
...
The next stage will continue improving runtime correctness, especially explicit human input / wait approval runtime semantics and more general Skill execution semantics.
