...
构建 ActorContext：身份、权限、工具、记忆、输入
  ↓
执行 Skill：判断、调用工具、等待人工输入、处理审批、生成结果
  ↓
...
当前版本在 v0.3.7 Trace Persistence 基础上，为 `human_input` 增加真实 Runtime 语义：ActorRuntime 执行到 `human_input` 时会返回 `waiting_human_input`，输出 `pendingHumanInput`；调用方通过 `continue({ type: "human_input_response" })` 提交人工输入后，Runtime 会把输入写入 `steps` / `outputs`，继续执行后续 `transform` / `return` 步骤。Trace 新增 `human_input_requested` 与 `human_input_received`，默认只记录输入元数据，不把完整人工输入值写入 Trace。
...
v0.3.8 中，Skill Runtime 的目标是让配置声明和运行行为继续对齐：
...
{{context.order_id}}
{{steps.ask_human}}
{{outputs.human_confirmation}}
{{outputs.triage_result.reason}}
```

完整占位符会保留原始类型，因此 `{{outputs.triage_result}}` 可以返回对象，`{{outputs.triage_result.should_create_ticket}}` 可以返回布尔值，`{{outputs.human_confirmation}}` 可以返回人工输入原值。

## Human Input Runtime

`human_input` 是 Skill 流程中的人工补充输入边界，不等同于工具审批。审批回答“这个 ToolCall 是否允许执行”，human input 回答“流程需要人类补充什么信息”。

```ts
const waiting = await actorRuntime.run({ actorConfig, skillConfig, input });

if (waiting.status === "waiting_human_input") {
...
当前记忆和 Trace 系统仍然是轻量级本地实现，但已经具备清晰边界：
...
TraceLogger
  ├─ dumpSnapshot()                 # 导出 TraceSnapshot
  └─ restoreSnapshot()              # 从 TraceSnapshot 恢复
...
从 v0.3.6 开始，调用方可以把 MemoryStore 交给 Runtime：
...
这意味着 Runtime 会在构建上下文前加载历史记忆，让旧经验参与本次检索；运行结束沉淀新记忆后，再保存新的记忆快照。同时，运行过程本身也可以通过 TraceSnapshot 保存下来，供之后审计、调试和复盘。

## 项目状态

当前仍是实验性 Actor Kernel。它的重点不是把所有能力一次做完，而是逐步明确边界：Actor、Skill、Tool、Policy、Approval、Trace、Memory、Store 都先形成最小可运行闭环，再逐步硬化。

下一阶段将继续沿着运行时正确性推进，优先处理显式 wait_approval 运行语义，以及更通用的 Skill 等待 / 恢复模型。
...
Build ActorContext: identity, permissions, tools, memory, input
  ↓
Execute Skill: judge, call tools, wait for human input, handle approval, return result
  ↓
...
This version gives `human_input` a real Runtime contract on top of v0.3.7 Trace Persistence. When ActorRuntime reaches `human_input`, it returns `waiting_human_input` with `pendingHumanInput`; callers can submit `continue({ type: "human_input_response" })`; Runtime writes the human value into `steps` / `outputs` and continues through later `transform` / `return` steps. Trace now includes `human_input_requested` and `human_input_received`, recording metadata by default without storing the full human response value.
...
| Human Input | A human-supplied input boundary inside a Skill run, supporting waiting_human_input / continue. |
...
| Trace | Event log for auditability, debugging, and acceptance checks, now persistable through `TraceStore`. |
| MemoryCandidate | A memory candidate extracted after a practice run. |
| MemoryRecord | An authoritative long-term memory record. |
| MemoryStore | Snapshot storage abstraction, currently implemented by `JsonMemoryStore`, and now bindable to the ActorRuntime run lifecycle. |
...
Starting in v0.3.8, Skill Runtime keeps aligning configuration declarations with runtime behavior:
...
SkillConfig
  ...
  ├─ human_input    → returns waiting_human_input and waits for continue to write steps / outputs
...
A full-placeholder template preserves raw values, so `{{outputs.triage_result}}` can return an object, `{{outputs.triage_result.should_create_ticket}}` can return a boolean, and `{{outputs.human_confirmation}}` can return the raw human input value.

## Human Input Runtime

`human_input` is a human-supplied input boundary inside the Skill flow, distinct from tool approval. Approval answers whether a ToolCall may proceed; human input supplies missing information, judgment, or context to the Skill.
...
if (waiting.status === "waiting_human_input") {
  const completed = await actorRuntime.continue(waiting.actorRunId, {
    ...
    response: {
      humanInputRequestId: waiting.pendingHumanInput.humanInputRequestId,
      value: "Continue drafting, but do not claim the refund is completed.",
      respondedBy: "human_operator",
      respondedAt: new Date().toISOString(),
    },
  });
...
## Memory, Trace, and Storage

The current memory and Trace systems are still lightweight and local, but their boundaries are explicit:
...
TraceLogger
  ├─ dumpSnapshot()                 # Export TraceSnapshot
  └─ restoreSnapshot()              # Restore from TraceSnapshot
...
Starting in v0.3.6, callers can hand the MemoryStore to Runtime:
...
Starting in v0.3.7, Trace can also be saved and restored independently:
...
The Runtime loads historical memory before context construction so past experience can participate in retrieval, then saves a fresh memory snapshot after new memory is crystallized. The run process itself can also be persisted through TraceSnapshot for later audit, debugging, and review.
...
The next stage will continue improving runtime correctness, especially explicit wait_approval runtime semantics and a more general Skill waiting / resume model.
